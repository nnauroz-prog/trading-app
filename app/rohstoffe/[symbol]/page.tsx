// Rohstoff-Detail-Seite. Continuous-Front-Month-Future + technische Indikatoren.
// Nutzt dieselbe Yahoo-History- und Indikatoren-Lib wie die Aktien-Detail-Seite.
...
export default async function RohstoffDetailPage({ params }: PageProps) {
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-5 p-4 pb-20 md:p-6">
      ...
      <header className="space-y-2 rounded-2xl border border-slate-800/80 bg-slate-900/40 p-5">
        <div className="flex items-baseline justify-between gap-3">
          <div className="min-w-0">
            <div className="text-[10px] font-semibold uppercase tracking-[0.25em] text-emerald-400">
              {commodity.emoji && <span className="mr-1">{commodity.emoji}</span>}
              {commodity.group}
            </div>
            <h1 className="truncate text-2xl font-bold tracking-tight text-white">{commodity.name}</h1>
            <div className="font-mono text-[11px] text-slate-500">{commodity.symbol} · {commodity.unit}</div>
          </div>
      ...
      <footer className="border-t border-slate-900 pt-4 text-[10px] leading-relaxed text-slate-600">
        Datenquelle: Yahoo Finance v8 Chart API (Continuous Front-Month Future, 1-Jahr-Tageskerzen, Cache 1h).
        Spot vs. Futures kann durch Roll-Effekte leicht abweichen. Keine Anlageberatung.
      </footer>
    </main>
  );
}
