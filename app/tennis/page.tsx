...
export default async function TennisPage() {
  const leagues = await getTennisFixtures();
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-5 p-4 md:p-6">
      ...
      <header className="space-y-1">
        <div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-emerald-400">Tennis</div>
        <h1 className="text-3xl font-bold tracking-tight text-white">ATP, WTA, Grand Slams</h1>
        <p className="text-sm text-slate-400">Kommende Matches der vier Grand Slams und der ATP/WTA-Tour. Live von TheSportsDB.</p>
      </header>
      ...
      {leagues.every((l) => l.next.length === 0 && l.last.length === 0) && (
        <section className="rounded-2xl border border-amber-500/30 bg-amber-950/15 p-4 text-[11.5px] leading-snug text-amber-100/85">
          Aktuell keine Tennis-Matches im Datenpool. TheSportsDB pflegt Tennis weniger lückenlos als Fußball — sobald ein großes Turnier (z. B. Wimbledon, US Open) läuft, erscheinen die Matches hier.
        </section>
      )}
            ...
            {today.length > 0 && (
              <div className="space-y-1">
                ...
                <ul className="space-y-1">
                  {today.map((f) => (
                    <li key={f.id} className="grid grid-cols-[auto_1fr] gap-2 rounded-md border border-amber-400/40 bg-amber-950/20 px-2.5 py-1.5 text-[11.5px]">
      ...
      <footer className="border-t border-slate-900 pt-4 text-[10px] leading-relaxed text-slate-600">
        Datenquelle: TheSportsDB. Aktualisierung alle 10 Minuten.
      </footer>
    </main>
  );
}
