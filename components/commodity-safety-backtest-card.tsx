// Backtest-Beleg für Rohstoffe — analog zur Aktien-Karte, eigene Tabelle pro
// Rohstoff (Gold, Silber, …).

import type { CommoditySafetyBacktestSummary } from '@/lib/market/commodity-safety-backtest-scan';

export function CommoditySafetyBacktestCard({ summary }: { summary: CommoditySafetyBacktestSummary }) {
  if (!summary.available) {
    return (
      <section className="space-y-2 rounded-2xl border border-slate-800/80 bg-slate-900/40 p-4">
        <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-400">Funktioniert die Rohstoff-Strategie? — Backtest</h2>
    ...
    <section className="space-y-3 rounded-2xl border border-slate-800/80 bg-slate-900/40 p-4">
      <div>
        <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-400">
          🧪 Backtest des Rohstoff-Sicherheits-Gates (12 Monate)
        </h2>
        <p className="text-[10.5px] leading-snug text-slate-500">
          Walk-forward über Continuous-Front-Month-Futures: an jedem Tag das 8-Punkt-Gate ausgewertet,
          bei Grade A simulierter Einstieg, Exit bei Verlust von Grade A. {summary.totalAssetsScanned} Rohstoffe geprüft.
        </p>
      </div>
      ...
      {summary.perAsset.length > 0 && (
        <details className="rounded-lg border border-slate-800 bg-slate-950/40">
          <summary className="cursor-pointer p-2.5 text-[11px] font-semibold uppercase tracking-wider text-slate-400 hover:text-slate-200">
            ▸ Performance pro Rohstoff ({summary.perAsset.length})
          </summary>
            ...
            <table className="w-full text-[10.5px]">
              <thead>
                <tr className="text-[9px] uppercase tracking-wider text-slate-500">
                  ...
                  <th className="py-1 text-left">Gruppe</th>
              ...
              <tbody>
                {summary.perAsset.map((a) => (
                  <tr key={a.symbol} className="border-t border-slate-800/60">
                    ...
                    <td className="py-1 truncate text-slate-500">{a.group}</td>
      ...
      <p className="text-[10px] leading-snug text-slate-500">
        Vergangenheit ≠ Zukunft. Backtest ohne Gebühren/Slippage/Roll-Effekte. Strategie schlägt nicht jedes Mal Buy-and-Hold — Stärke liegt in der Risiko-Adjustierung.
      </p>
    </section>
  );
}
