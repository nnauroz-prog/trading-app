...
// Krypto Precision Desk — Calibration-Hydrator.
//
// Liest das FirmaDecision-Log aus localStorage, evaluiert per-Coin Outcomes
// gegen die uebergebenen latestPrices und zeigt pro Probability-Bucket
// Sample-Groesse, historische Trefferquote, Label KALIBRIERT | UNKLAR |
// UEBERSCHAETZT. Wenn keine bewertbaren Trades existieren, versteckt sich
// die Karte ehrlich.

import { useEffect, useMemo, useState } from 'react';
import {
  FIRMA_DECISIONS_CHANGED_EVENT,
  loadFirmaLog,
  type FirmaDecision
} from '@/lib/firma-memory';
...
export function CryptoPrecisionCalibrationHydrator({ latestPrices }: Props) {
  ...
  const result = useMemo(() => {
    const priceFor = (coin: string) => latestPrices[coin.toLowerCase()] ?? null;
    return buildCryptoCalibrationFromFirmaLog(log, priceFor);
  }, [log, latestPrices]);

  if (!mounted) return null;
  if (result.totalResolvedTrades === 0) return null;

  const visible = result.stats.filter((s) => s.sampleSize > 0);
  if (visible.length === 0) return null;

  return (
    <section className="space-y-2 rounded-2xl border border-slate-800 bg-slate-900/40 p-3" aria-label="Krypto Precision Calibration Status">
      <div className="flex items-baseline justify-between gap-2">
        <h3 className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400">Kalibrierungs-Status (aus Firma-Trades)</h3>
        <span className="text-[10px] text-slate-500">{result.totalResolvedTrades} bewertet</span>
      </div>
        ...
        {visible.map((s) => {
          ...
          return (
            <li key={s.bucket} className={`grid grid-cols-[auto_auto_1fr_auto] items-center gap-2 rounded border px-2 py-1 text-[10.5px] ${LABEL_CLASS[s.label]}`}>
              ...
              <span className="text-[9.5px] opacity-70">{s.sampleSize} Trade{s.sampleSize === 1 ? '' : 's'}</span>
      ...
      <p className="text-[10px] leading-snug text-slate-500">
        Bewertete BUY-Entscheidungen Deiner Firmen — TP1 erreicht = Treffer, Stop ausgeloest = daneben. Wenn ein Bucket historisch unter Erwartung trifft, deckelt der Krypto Precision Desk zukuenftige Picks in diesem Bucket.
      </p>
    </section>
  );
}
