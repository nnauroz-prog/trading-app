...
interface Props {
  scanEntries: CommoditySafetyEntry[];
...
export function RohstoffWatchlistPageClient({ scanEntries, perAssetBacktests }: Props) {
  ...
  if (!mounted) {
    return <p className="rounded-2xl border border-slate-800/80 bg-slate-900/40 p-4 text-sm text-slate-400">Lade Watchlist …</p>;
  }

  if (items.length === 0) {
    return (
      <section className="rounded-2xl border border-slate-800/80 bg-slate-900/40 p-4 text-sm text-slate-400">
        Deine Rohstoff-Watchlist ist leer. Öffne eine Rohstoff-Detail-Seite (z. B. <Link href="/rohstoffe/GC%3DF" className="text-emerald-300 hover:underline">Gold</Link>) und klick auf &bdquo;&#9734; zur Watchlist&ldquo;.
      </section>
    );
  }
            ...
            <li key={w.symbol} className="space-y-1.5 rounded-2xl border border-slate-800/80 bg-slate-900/40 p-3">
              <div className="flex items-baseline justify-between gap-2">
                <div className="min-w-0">
                  <Link href={`/rohstoffe/${encodeURIComponent(w.symbol)}`} className="font-semibold text-slate-100 hover:text-emerald-300">
                    {w.name}
                  </Link>
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <button
                  onClick={() => removeRohstoffWatch(w.symbol)}
