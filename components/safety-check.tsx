    ...
    <section className="space-y-3 rounded-2xl border border-slate-800/80 bg-slate-900/40 p-5" aria-label="Sicherheits-Check">
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-[11px] text-slate-500">Strengste Stufe: grün nur, wenn ALLE Kriterien passen.</p>
        </div>
      ...
      {a.maxSafety ? (
        <div className="rounded-lg border border-emerald-400/50 bg-emerald-500/10 p-2.5 text-sm font-bold text-emerald-100">
          ✓ SICHERER KAUF — alle Sicherheits-Kriterien erfüllt
        </div>
      ) : (
      ...
      <p className="rounded-lg border border-slate-700/60 bg-slate-950/40 p-2.5 text-[10px] leading-relaxed text-slate-400">
        {a.residualRiskNote}
      </p>
    </section>
  );
