// Drei Persönlichkeiten geben für die heutige Aktien-Lage ihr Verdict ab.
// Analog zum Krypto-Vorstand, aber auf dem 8-Punkt-Sicherheits-Gate.

import Link from 'next/link';
import type { StockAgentVerdict } from '@/lib/agents/stock-personas';

const VERDICT_TONE: Record<StockAgentVerdict['verdict'], string> = {
...
export function StockPersonaPanel({ verdicts }: { verdicts: StockAgentVerdict[] }) {
  return (
    <section className="space-y-3 rounded-2xl border border-slate-800/80 bg-slate-900/40 p-4">
      <div>
        <h2 className="text-xs font-semibold uppercase tracking-wider text-emerald-300">👥 Aktien-Vorstand</h2>
        <p className="mt-1 text-[10.5px] leading-snug text-slate-500">
          Drei Persönlichkeiten — jede hat eigene Risiko-Profil-Regeln, eigenes Setup, eigenes Verdict.
          Wenn alle drei &bdquo;KAUFEN&ldquo; sagen: starkes Signal. Wenn sie auseinandergehen: aufpassen.
        </p>
      </div>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {verdicts.map((v) => (
          <article key={v.persona} className={`flex flex-col gap-2 rounded-xl border-2 p-3 ${VERDICT_TONE[v.verdict]}`}>
            ...
            {v.target ? (
              <Link
                href={`/aktien/${encodeURIComponent(v.target.symbol)}`}
                className="rounded-lg border border-current/30 bg-slate-950/40 p-2 transition hover:brightness-110"
              >
                <div className="flex items-baseline justify-between gap-2">
                  <span className="truncate text-[12px] font-bold">{v.target.name}</span>
